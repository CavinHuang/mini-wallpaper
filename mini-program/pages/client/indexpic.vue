		...
		<view class="integral-mall-main plr15" style="margin-top: 140upx;">
			<view class="integal-mall-menu flex pt16 pb16">
				<view class="col2 text-center" @click="linkTo"
					...
					<view>
						<image style="width: 140rpx; height: 80rpx; border-radius: 10upx;" mode="aspectFill"
							:src="statics.zhuicotp[0]"></image>
					</view>
					<view class="ft14 ftw600 mt6" style="color: #444;">小姐姐</view>
				</view>
				<view class="col2 text-center bd-left" @click="linkTo"
					data-link="/pages/client/tuan/sstp?selectIndex=4&typeids=101">
					<view>
						<image style="width: 140rpx; height: 80rpx; border-radius: 10upx;" mode="aspectFill"
							:src="statics.zhuicotp[1]"></image>
					</view>
					<view class="ft14 ftw600 mt6" style="color: #444;">小哥哥</view>
				</view>
				<!-- /pages/client/integral/role -->
				<view class="col2 text-center bd-left" @click="linkTo"
					data-link="/pages/client/tuan/sstp?selectIndex=4&typeids=104">
					<view>
						<image style="width: 140rpx; height: 80rpx; border-radius: 10upx;" mode="aspectFill"
							:src="statics.zhuicotp[2]"></image>
					</view>
					<view class="ft14 ftw600 mt6" style="color: #444;">精品头像</view>
				</view>
				<view class="col2 text-center bd-left" @click="linkTo"
					data-link="/pages/client/tuan/sstp?selectIndex=4">
					<view>
						<image style="width: 140rpx; height: 80rpx; border-radius: 10upx;" mode="aspectFill"
							:src="statics.zhuicotp[3]"></image>
					</view>
			...
			<view class="mt24 mb20">
				...
				<sub-tabvsw class="pt5" v-if="type4tab[1]" :tabs="type4tab" @change="changetype4"
					:selectIndex="selecttype4"></sub-tabvsw>
			...
			getList(lx, ishot) {
				...
				uni.request({
					...
					success: res => {
						...
						this.banners = res.data.config.banner
						if (res.data.config.site.mbgColor) {
							this.mbgColor = res.data.config.site.mbgColor
							uni.setNavigationBarColor({
								frontColor: "#ffffff",
								backgroundColor: this_.mbgColor,
								complete: () => {
						...
						this.type4tab = res.data.config.type4
						console.log(this.type4tab)
						if (res.data.data.total > 0) {
							this.dataconfig = res.data.config
							if (lx) {
								this.dataindex[lx] = res.data.data.rows
							} else {
								this.datasa = res.data.data.rows
							}
						}
					},
					fail: (data, code) => {}
