			...
			<view class="box pd16_15 alcenter space" style="margin-bottom: 30upx;">
				<view class="flex" style="width: 100%;" @click="detail(item.id)">
					<view style="width:230rpx;">
						<image mode="aspectFill" class="order-list-item-l" :src="imagePath(item.upload_type, item.thumb_url)"
							style="margin-right: 5%; border-radius: 5px;"></image>
					</view>
					<view style="width: 100%; padding-left: 20upx;">
						<view class="ft14 cl-main">{{item.name}} </view>
						<view class="mt8 ft12 cl-notice">
							￥{{item.price}}
							<text v-if="role==2" class="ft12 cl-notice" style="float: right;">UID:{{item.author}} </text>
						</view>
						<view class="mt8 ft12 cl-notice">
							{{item.create_at}}
							<text class="ft12 cl-notice" style="float: right;"> {{item.status==2?'未审核':'已审核'}}
							</text>
						</view>
						<view class="mt8">
							<view v-if="role==2" class="flex alcenter space mt16">
								<view class="btn-small plan" @tap.stop="tougaodel(item.id,index)">删除</view>
								<view @tap.stop="shenhe(item.id,index,item.status)">
									<view class="btn-small" v-if="item.status=='hidden'" :style="getBtnStyle">
										{{item.status==2?'审核':'取消审核'}}</view>
									<view class="btn-small" v-else style="background: #0dbbc3; color: #ffffff;">
										{{item.status==2?'审核':'取消审核'}}</view>
								</view>
							</view>
							<view v-else class="flex alcenter space mt16">
								<view class="btn-small plan" @tap.stop="detail(item.id)">详情</view>
								<view class="btn-small plan" :style="getBtnStyle" @tap.stop="tougaodel(item.id,index)">
									删除</view>
							</view>
						</view>
					</view>
				...
				uni.request({
					...
					success: data => {
						...
						if (data.data.data.total > 0) {
							...
							if (data.data.data.total < this.last_id * limit) {
								this.status = '';
							}
						} else {
							this.status = '';
