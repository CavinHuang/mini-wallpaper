 ...
 */
function trim(str: string, type = 'both') {
  if (type === 'both') {
 ...
 */
function getLengthUnitValue(value: string, unit = 'rpx') {
  if (!value) {
    return ''
  }
 ...
 */
function humpConvertChar(string: string, replace = '_') {
  if (!string || !replace) {
    return ''
  }
 ...
 * @param {Object} replace 进行连接的字符
 */
function charConvertHump(string: string, replace = '_') {
  if (!string || !replace) {
    return ''
  }
